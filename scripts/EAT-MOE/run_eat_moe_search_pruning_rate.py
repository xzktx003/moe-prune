...
DEFAULT_DATASETS = ("openbookqa", "arc_challenge", "mathqa")


def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Binary-search EAT-MOE tau by target pruning ratio using MCQA evaluation.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    def evaluate_tau(tau_value: float) -> Dict[str, object]:
        ...
        if not (args.skip_completed and json_result_has_payload(result_path)):
            ...
            if archived is not None:
                print(f"[EAT-search] archived incomplete work_dir {run_dir} -> {archived}", flush=True)
            cmd = [
                sys.executable,
            ...
            print("[EAT-search] eval ->", " ".join(cmd), flush=True)
            subprocess.run(cmd, cwd=str(EAT_ROOT), check=True)

        row = json.loads(result_path.read_text(encoding="utf-8"))[0]
        mathqa = float(row.get("mathqa_accuracy", 0.0))
        openbookqa = float(row.get("openbookqa_accuracy", 0.0))
        arc = float(row.get("arc_challenge_accuracy", 0.0))
        pruning_values = [
            float(row[f"{dataset_name}_pruning_ratio"])
            for dataset_name in args.datasets
            if f"{dataset_name}_pruning_ratio" in row
        ]
        accuracy_values = [
            float(row[f"{dataset_name}_accuracy"])
            for dataset_name in args.datasets
            if f"{dataset_name}_accuracy" in row
        ]
        record = {
            "tau": tau_value,
            "mathqa": mathqa,
            ...
            "arc_challenge": arc,
            "avg_accuracy": float(sum(accuracy_values) / len(accuracy_values)) if accuracy_values else 0.0,
            "avg_dynamic_pruning_ratio": float(sum(pruning_values) / len(pruning_values)) if pruning_values else 0.0,
            "wikitext_ppl": float(row.get("wikitext_ppl", 0.0)),
            "run_dir": str(run_dir),
    ...
    persist()
    print(f"[EAT-search] done search_dir={search_dir} records={len(records_by_tau)} targets={len(selections)}", flush=True)
    return 0
