...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Binary-search MoDES tau by target pruning ratio using WikiText PPL evaluation.')
...
def ensure_calibration(args: argparse.Namespace) -> Path:
    ...
    print('[MoDES-ppl-search] calibration ->', ' '.join(cmd), flush=True)
...
def resolve_result_path(run_dir: Path, tau_value: float) -> Path:
    per_tau_path = run_dir / 'ppl_by_tau' / format_tau_dir(tau_value) / 'wikitext_ppl.json'
    legacy_path = run_dir / 'wikitext_ppl.json'
    if per_tau_path.exists():
        return per_tau_path
    return legacy_path
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    def evaluate_tau(tau_value: float) -> Dict[str, object]:
        ...
        result_path = resolve_result_path(run_dir, tau_value)
        if not (args.skip_completed and json_result_has_payload(result_path)):
            archived = archive_incomplete_work_dir(run_dir)
            if archived is not None:
                print(f'[MoDES-ppl-search] archived incomplete work_dir {run_dir} -> {archived}', flush=True)
            result_path = run_dir / 'ppl_by_tau' / format_tau_dir(tau_value) / 'wikitext_ppl.json'
            cmd = [
                sys.executable,
            ...
            print('[MoDES-ppl-search] eval ->', ' '.join(cmd), flush=True)
            subprocess.run(cmd, cwd=str(MODES_ROOT), check=True)
        else:
            result_path = resolve_result_path(run_dir, tau_value)
        ...
        record = {
            'tau': tau_value,
            'ppl': float(row['ppl']),
            'avg_dynamic_pruning_ratio': float(row['pruning_ratio']),
            'rows_used': float(row['rows_used']) if row.get('rows_used') is not None else None,
            'windows': float(row['num_windows']) if row.get('num_windows') is not None else None,
    ...
    persist()
    print(f'[MoDES-ppl-search] done search_dir={search_dir} records={len(records_by_tau)} targets={len(selections)}', flush=True)
    return 0
