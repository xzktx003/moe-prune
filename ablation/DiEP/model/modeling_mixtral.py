...
class MixtralDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, router_logits, selected_experts = self.block_sparse_moe(hidden_states)
    ...
    def forward(
        ...
        for index, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                self.experts_frequency[index] = torch.zeros(8).float()
                #if decoder_layer.block_sparse_moe.gate.weight.size(0) !=8: #isinstance(decoder_layer.block_sparse_moe, MixtralSparseMoeBlock) and 
                for expert_id in range(selected_experts.size(0)):
                    try:
                        self.experts_frequency[index][selected_experts[expert_id]]+=1
                    except:
                        
                        self.experts_frequency[index] = self.experts_frequency[index].to(selected_experts.device)
                        try:
                            self.experts_frequency[index][selected_experts[expert_id]]+=1
                        except:
                            pdb.set_trace()
                # if decoder_layer.block_sparse_moe.gate.weight.size(0) !=8:
                
            # 
            # if isinstance(decoder_layer.block_sparse_moe, MixtralSparseMoeBlock) and decoder_layer.block_sparse_moe.gate.weight.size(0) !=8:
            #         pdb.set_trace()
            ...
            #         use_cache=use_cache,
            #     )
            #print('inference', type(decoder_layer.block_sparse_moe),decoder_layer.block_sparse_moe.gate.weight.size(0))
