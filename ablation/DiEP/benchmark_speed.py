...
from transformers.models.mixtral import modeling_mixtral #MixtralSparseMoeBlock, MixtralForCausalLM, MixtralDecoderLayer
...
def extract_data(data, preserve_experts):
    flat_list = [item for sublist in data for item in sublist]
    median_value = statistics.median(flat_list)
    #mean_value = np.mean(data)
    output=[]
    #pdb.set_trace()
    for expert in preserve_experts:
        
        #output.append([data[expert][id]/(median_value) for id in preserve_experts])
        output.append([data[expert][id]/0.7 for id in preserve_experts])
    
    return output

def read_data(txt_file, experts_to_perserve=None):
    data=[]
    items=[]
        ...
        for line in lines:
            ...
            if 'Layer' in line:
                ...
                if len(items) !=0:    
                    ...
                    if experts_to_perserve is not None and layer-1 in experts_to_perserve.keys():        
                        items= extract_data(items, experts_to_perserve[layer-1])
                    else:
                        items= extract_data(items, range(0, len(items)))
                    data.append(items)
                
                items=[]

            else: 
                items.append([float(num) for num in line.replace(',','').split()])
        
        if experts_to_perserve is not None and len(experts_to_perserve[layer])>0:                
            items= extract_data(items, experts_to_perserve[layer])
        else:
            
            items= extract_data(items, range(0, len(items[0])))
        data.append(items)
...
def transform_dynamic_skip(model: MixtralForCausalLM, experts_difference: list):
    assert isinstance(model, MixtralForCausalLM) or isinstance(model, modeling_mixtral.MixtralForCausalLM)
    
    if not hasattr(model.config, 'betas'):
        logger.info('Model does not perform dynamic skipping.')
        return
    else:
        logger.info('Enabling dynamic skipping...')
    ...
    for i, layer in enumerate(model.model.layers):
        if isinstance(layer.block_sparse_moe, MixtralSparseMoeBlock) or isinstance(layer.block_sparse_moe, modeling_mixtral.MixtralSparseMoeBlock):
            ...
            layer.block_sparse_moe = DynamicSkippingMixtralSparseMoeBlockWrapper(
                layer.block_sparse_moe, float(model.config.betas[str(i)]), experts_difference[i])
...
def load_model_tokenizer(
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        pretrained_model_name_or_path=model_name_or_path, #tokenizer_name_or_path or 
...
def main():
    ...
    parser.add_argument("--model_name_or_path", type=str,
                        default="/root/autodl-tmp/Mixtral-8X7B-v0.1")
