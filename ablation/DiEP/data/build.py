...
def build_calib_loader(dataset: str, tokenizer: PreTrainedTokenizer, max_block_size: int, n_blocks_for_stat: int, batch_size: int, num_workers: int, seed: int = 42):
    ...
    #n_blocks_for_stat2 = n_blocks_for_stat*2
    if n_blocks_for_stat > 0:  # Random choose `n_blocks_for_stat` blocks
        ...
        train_set = all_set['train'].shuffle(seed=seed).select(range(n_blocks_for_stat * 16, n_blocks_for_stat * 16*2))
    ...
    lm_train_set = tokenized_train_set.map(
        group_texts,
        batched=True,
    )
    ...
    if n_blocks_for_stat > 0:
        ...
        lm_train_set = lm_train_set.select(range(n_blocks_for_stat))
